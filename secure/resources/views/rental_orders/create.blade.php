...
@section('content')
    @php
        ...
        // Artikel-Labels zentral aus dem Catalog
        ...
        <div class="a4-form">
            ...
            <div class="row g-3 mt-2">
                <div class="col-md-6">
                    ...
                    <input class="form-control" name="customer_name" value="{{ old('customer_name') }}" @if($reqName) required @endif />
                </div>
                <div class="col-md-6">
                    <label class="form-label">Tel. {!! $reqPhone ? '<span class="text-danger">*</span>' : $optionalBadge !!}</label>
                    <input class="form-control" name="customer_phone" value="{{ old('customer_phone') }}" @if($reqPhone) required @endif />
                </div>

                @if($emailEnabled)
                    <div class="col-md-6">
                        <label class="form-label">E-Mail {!! $emailRequired ? '<span class="text-danger">*</span>' : $optionalBadge !!}</label>
                        <input class="form-control" name="customer_email" type="email" value="{{ old('customer_email') }}" @if($emailRequired) required @endif />
                    </div>
                @endif

                <div class="col-md-6">
                    <label class="form-label">Straße {!! $reqStreet ? '<span class="text-danger">*</span>' : $optionalBadge !!}</label>
                    <input class="form-control" name="customer_street" value="{{ old('customer_street') }}" @if($reqStreet) required @endif />
                </div>
                <div class="col-md-6">
                    <label class="form-label">Pers.-Ausweis-Nr. {!! $reqId ? '<span class="text-danger">*</span>' : $optionalBadge !!}</label>
                    <input class="form-control" name="customer_id_number" value="{{ old('customer_id_number') }}" @if($reqId) required @endif />
                </div>
                <div class="col-md-6">
                    <label class="form-label">Wohnort {!! $reqCity ? '<span class="text-danger">*</span>' : $optionalBadge !!}</label>
                    <input class="form-control" name="customer_city" value="{{ old('customer_city') }}" @if($reqCity) required @endif />
                </div>
                <div class="col-md-6">
                    <label class="form-label">Führerschein-Nr. {!! $reqDl ? '<span class="text-danger">*</span>' : $optionalBadge !!}</label>
                    <input class="form-control" name="customer_driver_license_number" value="{{ old('customer_driver_license_number') }}" @if($reqDl) required @endif />
                </div>
                <div class="col-md-6">
                    <label class="form-label">Kfz.-Kennzeichen {!! $reqPlate ? '<span class="text-danger">*</span>' : $optionalBadge !!}</label>
                    <input class="form-control" name="customer_license_plate" value="{{ old('customer_license_plate') }}" @if($reqPlate) required @endif />
                </div>
                <div class="col-md-3">
                    <label class="form-label">Datum {!! $reqRentalDate ? '<span class="text-danger">*</span>' : $optionalBadge !!}</label>
                    <input class="form-control" name="rental_date" type="date" value="{{ old('rental_date') }}" @if($reqRentalDate) required @endif />
                </div>
                <div class="col-md-3">
                    <label class="form-label">Rückgabedatum {!! $reqReturnDate ? '<span class="text-danger">*</span>' : $optionalBadge !!}</label>
                    <input class="form-control" name="return_date" type="date" value="{{ old('return_date') }}" @if($reqReturnDate) required @endif />
                </div>
            </div>
                        ...
                        @foreach($leftItems as $key => $label)
                            <div class="a4-row">
                                ...
                                <div class="a4-cell a4-qty">
                                    <input class="form-control form-control-sm" type="number" min="0" inputmode="numeric" pattern="[0-9]*" name="items[{{ $key }}]" value="{{ old('items.'.$key) }}"
                ...
                <div class="col-lg-6">
                    ...
                    <div class="a4-table">
                        @foreach($rightItems as $key => $label)
                            <div class="a4-row @if(in_array($key, $spuelItems)) spuel-item d-none @endif">
                                <div class="a4-cell a4-item">{{ $label }}</div>
                                <div class="a4-cell a4-qty">
                                    <input class="form-control form-control-sm" type="number" min="0" inputmode="numeric" pattern="[0-9]*" name="items[{{ $key }}]" value="{{ old('items.'.$key) }}" />
                                </div>
            ...
            @if($notesEnabled)
                <div class="row g-4">
                    <div class="col-12">
                        ...
                        <textarea class="form-control" name="notes" rows="3" placeholder="Notizen..." @if($notesRequired) required @endif>{{ old('notes') }}</textarea>
                    </div>
                </div>
            @endif
            ...
            <div class="row g-4">
                ...
                @if($signatureEnabled)
                    <div class="col-lg-6">
                        ...
                        <div class="form-text">Unterschrift wird als PNG gespeichert und an die Vermietung gekoppelt.</div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (spuelthekeInput) {
                ...
                if (parseInt(spuelthekeInput.value || 0, 10) > 0) {
                    syncSpuelVisibility();
                }
            }
        });
